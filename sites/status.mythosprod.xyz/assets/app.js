...
(function () {
  ...
  function renderLegend(d) {
    ...
    if ((d.source_hierarchy || []).length) {
      ...
      d.source_hierarchy.forEach(function (s) {
        ol.appendChild(el('li', { text: s.source + ' — ' + s.authority }));
      });
      root.appendChild(ol);
    }
  }
  ...
  function renderMatrix(d) {
    ...
    matrixRows(d).forEach(function (p) {
      ...
      tr.appendChild(el('td', { text: p.purpose }));
  ...
  function renderChanged(d) {
    ...
    if (!grid.childNodes.length) root.appendChild(el('p', { class: 'section-note', text: 'No changes recorded against the previous review.' }));
  ...
  function renderDocs(d) {
    ...
    (d.documents || []).forEach(function (doc) {
      ...
      var td = el('td'); td.appendChild(pill(doc.classification)); tr.appendChild(td);
  ...
  function renderRepos(d) {
    ...
    (d.repositories || []).forEach(function (r) {
      ...
      var td = el('td'); td.appendChild(pill(r.classification)); tr.appendChild(td);
    ...
    byId('btn-compare').addEventListener('click', function () {
      ...
      Promise.all([fetch(fa).then(function (r) { return r.json(); }),
        ...
        .then(function (pair) {
          ...
          if (!diffs.length) { out.appendChild(el('p', { class: 'section-note', text: 'No differences between the selected snapshots.' })); return; }
          var ul = el('ul', { class: 'plain' });

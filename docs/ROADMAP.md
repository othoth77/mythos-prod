...
| INF-DEPLOY-AUTO-0 | GitHub to Coolify Delivery Foundation | ◐ Contract ratified (O-DEPLOY-1/2/3), implemented + tested (2026-08-15), 90 tests — **staging only; no deployment executed.** No Coolify resource declares `environmentName=staging`, so the target cannot be proven; production is structurally unreachable from the executor. Operator action required. |

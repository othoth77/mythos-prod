...
| INF-BACKUP-AUTO-0 | Automated Backup and Restore Verification | ◐ Contract ratified 2026-08-16 by owner decisions **O-BACKUP-1..4 + O-BACKUP-5** (levels: `backup_create`/`restore_test` LEVEL_4 under the committed policy; no LEVEL_2 mutation, permanently); implemented and tested (`backup-operations-orchestrator.js` + 245 tests, 34/34 + 8/8 mutations caught), `backup_create` wired as pure delegation; **operationally enabled by O-BACKUP-6** (connector on with `secret_reference_id: secref-r2-backup`, LEVEL_2+LEVEL_4 flags on, LEVEL_3 off, 0600 credential file designated) — **no backup created, no restore performed**; only read-only `backup_verify` is authorised to execute. Wraps the existing `projects/infrastructure/ops/offhost-backup.js`; a parallel mechanism is refused in code. |
...
| STC-1b | gh-issue-140 recuration: fixed the stale PROJECT-AI-ORCHESTRATION entry (unchanged for 10 days across ~12 delivered stages), added 4 registered projects for `projects/*` directories the account-level discovery path structurally cannot see (mythos-gateway, mythos-orchestrator, mythos-vault, oth-mcp), and a monorepo project-discovery function (`engine.discoverMonorepoProjects`) mirroring the existing GitHub-repository discovery — every real `projects/*` directory now claimed, 0 unclassified. Tests stc-1 **81/0** (was 73). | ✓ Done (2026-09-03) |

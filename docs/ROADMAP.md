...
6. **Automation:** AUT-0 complete; INF-OVH-API-0, INF-CF-AUTO-0 and **INF-DNS-AUTO-1 (complete 2026-08-15)** are all mocked reference implementations (no live credential, not deployed, not connected to a live provider, no DNS record/zone/nameserver touched); AUT-CONNECTOR-SHARED-HELPERS-0 (code-quality cleanup, no behaviour change) complete; INF-DNS-AUTO-2 is the next Automation implementation stage — not started, requires its own authorisation (it would execute a `LEVEL_3_APPROVAL_REQUIRED` DNS operation), does not change items 1–4 above. **INF-DNS-AUTO-1 does not unblock INF-CF-2** — it reports on entry criteria 8 and 9 and can never report the gate as open.

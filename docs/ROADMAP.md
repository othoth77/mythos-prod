...
| INF-BACKUP-AUTO-0 | Automated Backup and Restore Verification | ◐ Contract ratified 2026-08-16 by owner decisions **O-BACKUP-1..4**; implemented and tested (`backup-operations-orchestrator.js` + 188 tests, 34/34 mutations caught) — **no backup created, no restore performed.** Wraps the existing `projects/idauto/ops/offhost-backup.js`; a parallel mechanism is refused in code. Gated shut by four independent conditions: the LEVEL_2 external-mutation ambiguity (fail-closed, owner decision required), connector `enabled:false`, `level_2_recommend_runs` false, and no credential in an approved secret store. |

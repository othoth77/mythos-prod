...
| INF-BACKUP-AUTO-0 | Automated Backup and Restore Verification | ✗ **BLOCKED at the contract gate (2026-08-16) — no contract exists.** Next in sequence, but scope is one sketching sentence: no designated automation level, no backup approval policy, no connector capable of backup creation or restore (`backup_storage_readonly` is `READ_ONLY`/disabled, `backup.list`+`backup.verify` only), no state model in the draft schema, and `docs/OFF_HOST_BACKUP_GATE.md` §6 records the subject matter as "not-yet-authorised". Working off-host backup tooling already exists and must not be rebuilt. Requires owner decisions **O-BACKUP-1..4**. |

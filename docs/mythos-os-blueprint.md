...
| 3G | js/plugins/production.runtime.js — Production runtime plugin. Primary business data plugin. onBoot validates 20 owned storage keys. onReady registers MythosSearch (order 10, 8 collections: invoices, clients, devis, contracts, rdvs, oms, representations, collabs) and MythosCalendar (order 10, mp_rdvs + mp_representations). All CRUD, rendering, business logic, sync engine, and dashboard statistics stay in app.js unchanged. | 9 703 | ✓ Complete |
| 3H | Runtime consolidation. All 7 runtime plugins migrated (Stages 3A–3G). Comprehensive architecture documented: docs/runtime-consolidation.md (plugin graph, provider registry, startup sequence, ownership map, legacy dependencies, dead code findings, future plan). Tests: tests/stage3h-test.js (8 sections). No new features, no code moves, no behavior changes. | 9 703 | ✓ Complete |

...
## Module Group: js/core/services/

**Status:** Stage 3A.5 — complete
**Load order:** blocking, after core/shell.js, before core/plugin-sdk.js
**Auto-wired by:** js/core/services/plugin-services.js

| File | Global | Purpose |
|---|---|---|
| search.js | `MythosSearch` | Text search provider registry |
| calendar.js | `MythosCalendar` | Calendar event provider registry |
| widgets.js | `MythosWidgets` | Dashboard widget registry (Shell compatible) |
| notifications.js | `MythosNotifications` | Unified toast + history |
| dialogs.js | `MythosDialogs` | Promise-wrapped alert/confirm/prompt |
| plugin-services.js | `PluginServices` | Bridges manifests to the services above |

All services are self-contained IIFEs (no cross-service dependencies except MythosWidgets → Shell and MythosNotifications → Events).

See `docs/runtime-services.md` for the complete API reference.

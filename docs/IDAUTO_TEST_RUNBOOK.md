...
| `IDAUTO_MEDIA_STORAGE_PATH` | `storage.js` | `/home/deploy/deployments/idauto-media` |
...
The canonical deployment file `/home/deploy/deployments/idauto-postgres/.env` (mode `600`, owner `deploy`) defines:
...
`/home/deploy/deployments/idauto-media` is `drwxr-x---  deploy deploy` — mode `750`. Only `deploy` (and root) can read or write it, so **IDA-2F and IDA-2H must run as `deploy`**. Running them as another user produces filesystem errors that look like storage bugs.
...
Run from `/home/deploy/projects/mythos-prod` as `deploy`.
...
set -a; . /home/deploy/deployments/idauto-postgres/.env; set +a
...
export IDAUTO_MEDIA_STORAGE_PATH=/home/deploy/deployments/idauto-media
...
**Rules are first-match-wins in array order.** Any new, more specific `projects/idauto/...` rule MUST be inserted **above** the general `projects/idauto/` rule, or it will never match. `tests/devx-1-idauto-test-impact-test.js` enforces this.

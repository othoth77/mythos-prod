...
| INF-BACKUP-AUTO-0 | Automated Backup and Restore Verification | ◐ Contract ratified by owner decisions **O-BACKUP-1..4** (2026-08-16) and **O-BACKUP-5** (2026-08-16, level designation: `backup_create`/`restore_test` are LEVEL_4_FULL_AUTOMATIC under the committed approved policy `inf-backup-auto-0-approval-policy.json`; no LEVEL_2 external mutation, permanently). Implemented and tested (`backup-operations-orchestrator.js` + 245-test suite, 8/8 O-BACKUP-5 mutations caught on top of the original 34/34), with the `backup_create` APPLY path wired as pure delegation to `projects/infrastructure/ops/offhost-backup.js`. **Operationally enabled by O-BACKUP-6 (2026-08-16)**: connector enabled with credential-by-reference `secref-r2-backup`, LEVEL_2 + LEVEL_4 flags on, LEVEL_3 flag kept off, the existing 0600 VPS credential file designated. **No backup created, no object uploaded, no restore performed**; only `backup_verify` (read-only) is authorised to execute, and each further operation needs its own owner order. *(Earlier blocker states — the LEVEL_2 ambiguity, then the three configuration conditions — were resolved by O-BACKUP-5 and O-BACKUP-6 respectively.)* |
...
**O-BACKUP-5 — LEVEL DESIGNATION (ratified 2026-08-16, resolving the LEVEL_2 external-mutation contradiction).**

> I approve option (a).
>
> backup_create and restore_test are designated
...
> - retention_report
>
> under the existing O-BACKUP-1 scope and safety boundaries.
>
> Permanent LEVEL_3 boundaries remain withheld:
> - production overwrite
> - backup deletion
> - disabling backups
>
> No LEVEL_2 external mutation is permitted.

*(Option (a) was one of three resolution options put to the owner by the 2026-08-16 blocker audit; it is the option the committed taxonomy itself anticipated — `docs/AUTOMATION_APPROVAL_MATRIX.md` §3 lists backup generation, backup integrity checks and isolated restore tests as the LEVEL_4-eligible operations, and this stage's original one-line description already cited that eligibility. The approved policy is committed as `projects/automation/config/inf-backup-auto-0-approval-policy.json` — two records: LEVEL_2 for the read-only operations, LEVEL_4 with the structural requirements of the committed LEVEL_4 definition for the mutating ones. The read operations `backup_verify` and `retention_report` remain LEVEL_2_RECOMMEND, whose "no external mutation" definition they genuinely satisfy — and that definition is now ratified as permanent: no LEVEL_2 operation may ever mutate.)*

**O-BACKUP-6 — OPERATIONAL ENABLEMENT (ratified 2026-08-16) — recorded verbatim.**

> I authorize exactly the following:
>
> 1. Set backup_storage_readonly.enabled = true.
> 2. Add: secret_reference_id = "secref-r2-backup"
> 3. Keep permission = READ_ONLY.
> 4. Keep capabilities EXACTLY: ["backup.list", "backup.verify"]
> 5. Set: level_2_recommend_runs = true
> 6. Set: level_4_full_automatic_runs = true
> 7. Keep: level_3_approval_required_runs = false
> 8. Designate the existing: ~/.config/mythos/idauto-offhost.env, mode 0600, as the credential target for: secref-r2-backup

*(The same decision authorised exactly one live operation — `backup_verify` only, read-only — after the configuration commit was pushed and verified; `backup_create`, `restore_test` and retention remain unexecuted pending their own orders. The credential file designated in point 8 lives outside the repository; its value is never read into any output, envelope or record — the committed catalogue carries the reference name only.)*
...
2. **No scope** — no inputs, outputs, entry criteria, completion criteria, prohibited operations, trigger model, schedule model, retention policy or state model.
3. **No designated automation level.** `LEVEL_4` *eligibility* is cited from `docs/AUTOMATION_APPROVAL_MATRIX.md` §3, but §4 of that document states the list "is a **candidate list, not a grant**" absent an explicit approved policy for each item — **no backup approval policy exists.**
4. **No capable connector.** `projects/automation/config/automation.example.json` holds exactly one backup connector, `backup_storage_readonly` (`permission: READ_ONLY`, `enabled: false`, capabilities `backup.list` and `backup.verify`). There is **no `backup.create`, `backup.restore`, retention or deletion capability anywhere in the catalogue**, so the catalogue cannot express this stage's own one-line scope.
5. **No state model.** The 24-table draft control-plane schema contains **zero** backup tables — its only mention of backups is a comment declaring the schema stores "No binary backup data."
6. **Explicit committed non-authorisation.** `docs/OFF_HOST_BACKUP_GATE.md` §6: *"recurring backups, retention automation and Coolify integration are separate, **not-yet-authorised** work."*

**This stage is not greenfield, and that is the crux.** `docs/OFF_HOST_BACKUP_GATE.md` §0 records that a working, provider-neutral, S3-compatible off-host backup implementation already exists (`projects/infrastructure/ops/offhost-backup.js`, `projects/infrastructure/ops/adapters/s3-compatible.js`, `tests/ida-3f-offhost-backup-test.js`), that the gate is **CLOSED** as of 2026-08-14 with 3/3 round-trip and 3/3 isolated-restore verification, and instructs in bold to **not rebuild it** — a second mechanism "would create two backup paths with one set of guarantees between them." INF-BACKUP-AUTO-0 therefore cannot be defined until the owner decides what it *is* relative to that tooling.

**Required owner decisions — the stage is executable immediately once these exist, and nothing else is blocking:**

- **O-BACKUP-1 — SCOPE.** Sources in scope (the three in `OFF_HOST_BACKUP_GATE.md` §1 or a subset), what a backup operation and a restore-test operation each are, entry/completion criteria, verification semantics, prohibited operations — and **explicitly, the relationship to the existing `offhost-backup.js` implementation** (extend / wrap / schedule; a parallel mechanism is forbidden).
- **O-BACKUP-2 — AUTOMATION LEVEL AND APPROVAL POLICY.** Per-operation level plus the approved policy §4 of the approval matrix requires before eligibility becomes a grant. The permanent `LEVEL_3` boundaries at §2 items 6–8 (production data overwrite, deletion of backups, disabling backups) are **not open to this stage**.
...
Implemented as `projects/automation/reference/backup-operations-orchestrator.js` + `tests/inf-backup-auto-0-backup-test.js` (originally 188 tests, 34/34 mutations caught; grown to **243 tests** with **8/8 further mutations caught** by the O-BACKUP-5 amendment of 2026-08-16, which also wired the `backup_create` delegation path — see the O-BACKUP-5 implementation status below).

**The module owns no backup logic.** It `require`s `projects/infrastructure/ops/offhost-backup.js` and delegates every manifest, checksum, upload, remote-verification, restore-verification and retention operation to it — O-BACKUP-1's reuse contract, enforced in code (`GATE_CHECK` refuses any plan whose `reuses_module` is not that path, and refuses any plan declaring `creates_parallel_mechanism`) and proven by test (the orchestrator `require`s no `fs`, no `http`/`https`, no `child_process`, and reimplements no checksum or manifest logic). Everything it adds is a gate in front of that tooling.
...
**THE ONE REMAINING AMBIGUITY — recorded, not resolved.** *(SUPERSEDED 2026-08-16 by O-BACKUP-5, above — the owner chose option (a): the two mutating operations were designated LEVEL_4_FULL_AUTOMATIC, the taxonomy was left untouched, and "no LEVEL_2 external mutation" became a permanent ratified rule. The paragraph below is retained as the record of the contradiction as it stood.)* O-BACKUP-2 assigns **LEVEL_2** to backup creation and to isolated restore testing. The repository's committed definition of that level (`projects/automation/config/automation.example.json` §`automation_levels`) is *"Analyse, create plans, simulate, dry-run, calculate impact, generate rollback plans. **No external mutation.**"* Backup creation writes objects to off-host storage and a restore test writes files to its target — both external mutations, which that definition excludes. The owner's gloss ("approved automation with bounded scope") and the committed definition do not agree. Per the standing instruction to stop at exactly such an ambiguity rather than invent a solution, this was encoded as a fail-closed constant `LEVEL_2_MAY_MUTATE_EXTERNALLY = false`.

**O-BACKUP-5 implementation status (2026-08-16):** `backup_create` and `restore_test` are `LEVEL_4_FULL_AUTOMATIC` in the orchestrator's constant table; `backup_verify` and `retention_report` remain `LEVEL_2_RECOMMEND`. The mutation gate (`assertMutationPermitted`) now requires all three of: LEVEL_4 designation, `level_4_full_automatic_runs === true`, and the committed approved policy covering the exact operation with the structural fields the committed LEVEL_4 definition demands (monitoring, audit, bounded retries, rollback/safe failure, the approving decision named). The run gate is level-aware: read-only operations run under `level_2_recommend_runs`, mutating operations under `level_4_full_automatic_runs`. The `backup_create` APPLY arm — previously an unconditional `UNREACHABLE_MUTATION_PATH` refusal — is now wired as pure delegation: `offhost.stage`/`verifyLocal` → `offhost.push` (COMPLETE marker last) → `offhost.verifyRemote`, behind a write-adapter boundary that refuses any deletion-shaped or undeclared method (the raw S3 adapter's `del` makes it refusable as-is; a composition root must wrap `{put, head, get}`) and stage-path guards that refuse the repository, `/etc`, `/var/lib` and `/`. Suite **243/243**, mutation checks **8/8 caught** (one initial survivor — a wrong-flag connector-gate mutant masked by the downstream mutation gate emitting the same code — was fixed by pinning the connector layer in isolation, tests 45a/45b).

**O-BACKUP-6 status (2026-08-16):** the three operational blockers are resolved by ratified configuration — `backup_storage_readonly` is `enabled:true` with `secret_reference_id: "secref-r2-backup"` (permission and capabilities byte-unchanged), `level_2_recommend_runs` and `level_4_full_automatic_runs` are `true`, `level_3_approval_required_runs` remains `false`, and the existing 0600 VPS credential file is the designated reference target. The suite asserts the ratified state directly: against the real committed catalogue and real committed flags, the read gate now passes. Every operation still runs only through the full gate chain, and only `backup_verify` has been authorised to actually execute. *(This paragraph previously recorded the pre-O-BACKUP-6 blocked state — "the gate refuses with CONNECTOR_DISABLED" — true until 2026-08-16.)*

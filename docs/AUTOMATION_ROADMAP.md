...
| INF-BACKUP-AUTO-0 | Automated Backup and Restore Verification | ◐ Contract ratified by owner decisions **O-BACKUP-1..4** (2026-08-16) and **O-BACKUP-5** (2026-08-16, level designation: `backup_create`/`restore_test` are LEVEL_4_FULL_AUTOMATIC under the committed approved policy `inf-backup-auto-0-approval-policy.json`; no LEVEL_2 external mutation, permanently). Implemented and tested (`backup-operations-orchestrator.js` + 243-test suite, 8/8 O-BACKUP-5 mutations caught on top of the original 34/34), with the `backup_create` APPLY path now wired as pure delegation to `projects/idauto/ops/offhost-backup.js`. **No backup created, no object uploaded, no restore performed.** Blocked from running by three remaining conditions, each an owner/operator action: `backup_storage_readonly` is `enabled:false` and carries no `secret_reference_id`; `level_2_recommend_runs` and `level_4_full_automatic_runs` are both false; no credential reference has been designated. *(The fourth blocker — the LEVEL_2 external-mutation ambiguity — was resolved by O-BACKUP-5.)* |
...
**No operation can run today:** `backup_storage_readonly` is `enabled:false` in the committed catalogue (and carries no `secret_reference_id`), `level_2_recommend_runs` and `level_4_full_automatic_runs` are both `false`, and no credential reference has been designated. The suite asserts these committed facts directly — against the real catalogue entry the gate refuses with `CONNECTOR_DISABLED`.

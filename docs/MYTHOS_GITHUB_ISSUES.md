...
| `Depends on: gate-<name>` | a **start gate** (`projects/mythos-ai-executor/bridge/gates/gate-<name>.json`): the bridge does not claim the task until it proves every requirement itself — commits/files on main, read-only live probes. Prose such as "do not start before …" in the body is never evaluated. See `bridge/gates/README.md` |

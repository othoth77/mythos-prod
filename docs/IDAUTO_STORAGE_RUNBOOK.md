...
**Companion documents:** [`IDAUTO_TEST_RUNBOOK.md`](IDAUTO_TEST_RUNBOOK.md) (environment and test execution) · [`IDAUTO_ARCHITECTURE.md`](IDAUTO_ARCHITECTURE.md).
...
|---|---|---|---|
| Live media store | `/home/deploy/deployments/idauto-media` | `750` | `deploy:deploy` |
| Media objects | `…/aa/bb/<hash>` | `640` | `deploy:deploy` |
| Media backups | `/home/deploy/backups/idauto-media-backup-<UTC-timestamp>/` | `700` dir, `600` files | `deploy:deploy` |
| PostgreSQL backups (IDA-2B) | `/home/deploy/backups/idauto-postgres-<date>/` | root-only, `600` | `root` |
| Restore testing | `/home/deploy/restore-test/…` | `750` | `deploy:deploy` |

All media operations run **as `deploy`**. The store is mode `750`, so no other unprivileged account can read it.

## 3. Commands

All commands run from the repository root as `deploy`, with the environment loaded per [`IDAUTO_TEST_RUNBOOK.md`](IDAUTO_TEST_RUNBOOK.md) §3. `audit` and `backup` need database access; `verify-backup`, `restore-dry-run` and `restore` do **not**.

```bash
# Read-only integrity report (DB <-> filesystem)
node projects/idauto/ops/media-ops.js audit
node projects/idauto/ops/media-ops.js audit --json     # machine-readable

# Create a verified backup (default destination /home/deploy/backups)
node projects/idauto/ops/media-ops.js backup
node projects/idauto/ops/media-ops.js backup --dest /home/deploy/backups

# Re-verify a backup already on disk
node projects/idauto/ops/media-ops.js verify-backup --backup <backup-dir>

# Show exactly what a restore would do — writes nothing
node projects/idauto/ops/media-ops.js restore-dry-run --backup <backup-dir> --dest <dir>

# Restore into an explicit destination
node projects/idauto/ops/media-ops.js restore --backup <backup-dir> --dest <dir>
```
...
- `restore` refuses the live media store, any path inside it, protected system paths (`/`, `/home`, `/home/deploy`, `/etc`, `/var`, `/usr`, `/root`, …), and any path inside the git repository.
...
```bash
# Latest backup
ls -1dt /home/deploy/backups/idauto-media-backup-* | head -1

# Verify it BEFORE you ever need it — an unverified backup is not a backup
node projects/idauto/ops/media-ops.js verify-backup \
  --backup "$(ls -1dt /home/deploy/backups/idauto-media-backup-* | head -1)"
...
| Location | `/home/deploy/backups/idauto-media-backup-<ts>/` | `/home/deploy/backups/idauto-postgres-<date>/` |
...
node projects/idauto/ops/media-ops.js restore --backup <backup-dir> --dest /home/deploy/restore-test/idauto-media-<date>
...
| **Whole server loss** | Provision the host, restore the PostgreSQL dump and the media backup **(media first, §7)**, restore `IDAUTO_MEDIA_STORAGE_PATH` ownership to `deploy:deploy` `750` and objects to `640`, then `audit`. Note that both backup sets currently live **on the same host** — see §11. |
...
- **Directory mode drift.** Media subdirectories are a mix of `755` and `775` (both `deploy:deploy`), reflecting different umasks at creation time. The store root is `750`, so nothing outside `deploy` can traverse them, and object files are consistently `640`. Not a security issue; recorded for completeness.

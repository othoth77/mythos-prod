...
## 14. Provider contract and ledger hardening (COMMS-9)

**Contract** (`reference/comms/provider.js`): every transport is a module satisfying `id`, `describe()`, `capabilities()`, `parseInbound(body)`, `sendText(o)`, `fetchMedia(o)`, `verifyWebhook(req, ctx)`, `health(o)`, `payloadHash(raw)`, `redactDeep(v)`; `register()` refuses non-conforming modules with named problems; `can(providerId, 'media.fetch')` is the lookup the Core uses instead of branching on provider names. The capability descriptor carries `channel`, `official`, `text`, `media { inbound, outbound, fetch, kinds }`, `templates`, `reactions`, `quotes`, `conversation_window_hours`, `signed_webhooks`, `webhook_retries`, `delivery_states`, `limitations`. Evolution today: unofficial, unsigned, no retries, no templates, media fetch off (Phase H). `GET /api/comms/providers` exposes the registry (non-secret). Contract suite `tests/mythos-wp-comms-contract-test.js` runs the same checks against Evolution and the fake fixture `tests/fixtures/comms/fake-provider.js` (a signed provider).

**Verification**: the receiver asks the provider. Unsigned providers are verified before the body is read with the shared 0600 token (Evolution behaviour unchanged: header `x-mythos-webhook-token` or `?token=`, constant-time compare); providers that declare `signed_webhooks` are verified after the body is read with the raw body and a secret the deployment supplies (`deps.secretFor(providerId)`), which is how the Meta HMAC adapter will plug in without weakening the token path.

**Identities** (migration 0005): `wp_contact_identities (project_id, contact_id, kind phone|lid|bsuid|provider_user, value, provider, verified_at)`, unique per `(project, kind, value)`; `wp_contacts.wa_id` is nullable and unique only when present. `core.resolveContact()` resolves any known identity first, then a legacy phone, else creates the contact and attaches every identity it saw; later identifiers attach to the same contact. Existing contacts were backfilled (phone and LID rows).

**Neutral events** (`reference/comms/events.js`): `message.received|sent|delivered|read|failed|status|duplicate|dry_run|note`, `conversation.created|updated|assigned|resolved|reopened|read`, `contact.created|identity_added|merged`, `handoff.created|resolved`, `ai.run|suggested|decided|sent|refused`, `delivery.alarm`, `inbox.status|heartbeat`, `event.rejected|replayed` — written to `event_name` on both ledgers and carried as `event` on the SSE bus.

**Ordering**: timeline and previews order by `COALESCE(provider_timestamp, created_at)`, then `created_at`, then `id` (index `wp_messages_order_idx`).

**Assistant gate**: `assistant.suggest()` refuses (412) when the conversation is `needs_human` or has an open handoff, journals `ai.refused`, creates no run.

**Reconciliation, heartbeat, replay** (`reference/comms/reconcile.js`, CLI `mythos-wp comms …`): `reconcile` stamps `ack_alarm_at` once on outbound rows still `queued`/`sent` after `MYTHOS_WP_ACK_THRESHOLD_MIN` (default 15) and journals `delivery.alarm` — never resends; `heartbeat` probes every non-inactive inbox through `provider.health()` and records `heartbeat_state ok|stale|unreachable` (audited on change, `inbox.status` untouched); `replay <event_id>` re-runs the Core on one ledgered failed/rejected delivery whose redacted payload was kept — dry-run unless `--apply`, idempotent through the unique provider id, audited in `wp_audit_events`, never accepts a payload from the caller.

## 15. Shared WhatsApp account mode (COMMS-11)

One account = one Evolution instance = one session; the instance identifies the **session**, never the service. An instance declared `shared` (`wp_inboxes.account_mode`, `account_ref` mandatory, reserved account only with the audited `allow_personal_account` opt-in) may host several logical inboxes, one per project. Inbound messages pass `routing.resolve()` **before any ledger row**: an enabled `wp_inbox_routes` rule for one of the sender's identities (`allowlist`, or `opt_in` = pre-registered identity + optional code + window) routes to exactly one inbox; anything else is dropped with hashes only (`wp_routing_drops`) — no contact, conversation, message, payload or dead-letter. Default deny, no default inbox, owner/reserved numbers never routed, `mythos-bridge` allowed only in shared mode, dedicated instances keep the COMMS-1..9 behaviour. Delivery/connection events address the instance's inboxes; the bridge's own notifications (sent to self) are never ingested. Phone-side replies by the owner are own messages and are not mirrored (documented limitation). Full rationale: research artifact §15.

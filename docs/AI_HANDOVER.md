...
1. **Close the documentation debt** — correct the now-false `NOT DEPLOYED` / `DRY-RUN` headers in `schema.sql`, `import.sql`, `validation.sql` and `migration/README.md`. Blocked on filesystem ownership; needs a session as `ubuntu` or an owner `chown`/ACL change on `projects/ssangyong-autos/`.

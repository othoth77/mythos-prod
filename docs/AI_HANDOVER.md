...
0. **Notification quota — the one open item.** ntfy.sh free-tier rate limiting (HTTP 429, sustained across 20 retries over 20 minutes) is suppressing delivery. Confirm with a single `projects/mythos-orchestrator/notify.sh task_completed <stage> <detail>` once the daily quota resets and check `logs/notify.log` records `outcome=sent`. If notifications matter operationally, the durable fix is an authenticated or self-hosted ntfy instance — the free tier's per-IP daily cap is easy to exhaust across a busy orchestration day, and the failure is silent by design (non-fatal, exit 0), so it will not announce itself.

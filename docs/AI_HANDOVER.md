...
- **MERGED ≠ RUNNING:** this entry is the code stage. The live checkout `~/projects/mythos-prod` must carry this commit **before** the unit is enabled, or the scheduled health timer (running main's `haddad-health.js`) FAILs on the 8160 listener. The live enablement, `--serve` outcome and health/telemetry numbers are recorded in the next entry.
- **Next Claude:** `docs/HADDAD_MCP.md` §12. Do not bind the bridge anywhere but loopback; do not use Funnel; do not add a second variable to `mcp-http.env` (three checks refuse it: setup, health, test).

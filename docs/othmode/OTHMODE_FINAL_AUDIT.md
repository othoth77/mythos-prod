...
| 17 | OthMode ON/OFF | **COMPLETE** | Owner-role HTTP + operator CLI; fail-closed (absent store = OFF); ON→OFF cycle exercised live on production (left OFF for the owner); each flip recorded as an evolution event; CLAUDE.md instruction contract added |

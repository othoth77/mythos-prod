...
| Skill trust (SKILL-TRUST-0, 2026-09-04) | **Implemented.** A skill is executable only with a content-bound `ACCEPT` attestation produced by NVIDIA SkillSpector + Gitleaks + NVIDIA SkillEvaluator Tier 1 through the OTHMODE policy (`data/skill-trust-policy.json` → `ACCEPT / REVIEW / BLOCK`). The executor verifies the ledger hash at load (`lib/skill-trust.js`); OTHMODE shows the same status per skill and per MCP server (`GET /api/othmode/trust`) and never writes a ledger over HTTP — attestations are operator-CLI runs committed as diffs. No scan ≠ safe; scanner failure ≠ safe; unknown → REVIEW; DO_NOT_INSTALL → BLOCK. See `docs/OTHMODE_SKILL_TRUST.md` |

...
**`IDA-2F` — Object Storage Wiring — ✓ COMPLETE (2026-08-11):**
- `projects/idauto/reference/storage.js` — new. **Not** the `mythos_documents` integration contract in `docs/IDAUTO_ARCHITECTURE.md` §4.3 (Provider: "Mythos OS document storage", Activation: IDA-4, for professional service-event attachments and Fixpert carte grise) — confirmed no real cloud object storage (S3/MinIO/R2) exists anywhere in this codebase or on this VPS. Instead follows the pattern the main Mythos OS app already uses for file storage (`upload.php` → a local directory): content-addressed local filesystem storage (`SHA-256(content)` as the key, nested two levels deep), kept in its own directory `/home/deploy/deployments/idauto-media/` — outside this repository and outside `upload.php`'s own `/documents/` directory, so it never collides with or depends on the main app's storage.
- New routes: `POST /api/observations/:id/media` (raw binary body; `Content-Type` header is the mime type; optional `X-Idauto-Media-Type`/`X-Idauto-Access-Scope`/`X-Idauto-Blurred` headers), `GET /api/observations/:id/media` (metadata only — object_key is a storage reference, never a fetchable URL; no image-serving path exists in this stage).
- The write goes through `writes.js`'s existing `withAudit()` exactly like every other mutation — same atomic guarantee, same real-identity `actor_ref`. The read excludes `mythos_private`-scope rows, same unchanged policy as every other GET in this file (the schema's own default `access_scope` for this table is `mythos_private`, so most rows are excluded by default unless a caller explicitly widens scope on write).
- **A genuinely new atomicity dimension, handled explicitly**: a filesystem write cannot participate in a Postgres transaction. The observation-exists check runs *before* `storage.store()` touches disk (a request for a nonexistent observation never writes a file at all). If the DB+audit insert fails *after* a successful disk write, the file is removed — but only after confirming no *other* `idauto_observation_media` row still references the same content-addressed key (since identical bytes uploaded for two different observations produce the same key; a naive unconditional delete could remove a file a different, already-committed row still needs). Proven by test in both directions: an unreferenced orphan gets cleaned up; an already-referenced file survives a subsequent failed attempt.
- `tests/ida-2f-object-storage-test.js` — new, **31/31 passing**, live against `idauto-postgres` and the real local filesystem. Self-caught the same class of bug IDA-2D found: the first test content used a static seed, which passed once but broke on re-run against the persistent database (an absolute "exactly 2 rows for this key" count assertion doesn't survive a second run if the content — and therefore its hash — never changes). Fixed the same way: `Date.now()`-seeded content; confirmed idempotent across 3 consecutive runs afterward.

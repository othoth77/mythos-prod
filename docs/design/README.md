...
| 15 | `MIG_EXECUTION_MAPPING.md` | **Mapping and tooling, not execution.** The real, measured scope of MIG-1/MIG-2 (42 and 93 occurrences — both larger than previously recorded) and a real pixel-diff pilot result for MIG-1's CSS-property layer. `../../tools/visual-verify.js`, the local-only tool it documents, is new (**AUTO-6**) |

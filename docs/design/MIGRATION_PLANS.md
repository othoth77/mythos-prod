...
| **MIG-2 — EXECUTED, AUTO-11 (2026-08-19)** | Solved by role separation, not type-shrinking: 44 display sites → Archivo Expanded 600, 6 label sites → Plex Sans 500, 43 numeric/financial sites → IBM Plex Mono 400 (the approved Data role — tabular figures for money, exactly what the system specifies them for). The AUTO-8 KPI wrap is gone: "6545.000 TND" renders on one line, cards aligned, verified across 17 real views. Playfair CDN removed; approved faces self-hosted. Inter body face remains — separate recorded step. See `MIG_EXECUTION_MAPPING.md` §3a-b, register AUTO-11 |
| **MIG-3 — COMPLETED, AUTO-9 + AUTO-12 (2026-08-19)** | `--muted`/`--danger` (AUTO-9); `--past` mapped to the approved disabled role `ink-400` with its 12% companion, and A-016's `#726F64` control boundary applied to the `input,select,textarea` rule plus 22 inline-styled form controls (decorative `#333` borders deliberately left — A-016 governs control boundaries, not decoration). Console kept in lockstep by the drift rule, 680/680. Plus A-022 44px nav touch floor and A-018 reduced-motion support, closing the sweep's two findings. See `MIG_EXECUTION_MAPPING.md` §4, register AUTO-12 |

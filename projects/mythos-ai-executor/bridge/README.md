...
| `requested_action` | `investigate` \| `review` \| `test` → read-only profile; `document` \| `implement` → repo-write profile. No provider, path, tool, MCP server or credential can be named in a task |
| `model` | optional. `Haiku` \| `Sonnet` \| `Opus` \| `Fable 5` (or a full id such as `claude-sonnet-5`) → that model runs, never a substitute; an unknown or unavailable value is refused with the accepted list. Omit it and the executor scores the task and picks Haiku, Sonnet or Opus — Fable is never chosen automatically. The chosen model and the reason appear in the report. Choosing a model grants no authority |

...
| `requested_action` | `investigate` \| `review` \| `test` → read-only profile; `document` \| `implement` → repo-write profile. Nothing else is selectable: no provider, model, path, tool, MCP server or credential can be named in a task |

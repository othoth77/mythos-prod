'use strict';
// =====================================================
// Mythos AI Executor — governed host operations (HOSTOPS-1)
...
//   6. the boundary                          — spawnSync of `sudo -n` + the
//      fixed helper path + an ARGUMENT ARRAY. No shell exists anywhere in
//      this path; there is no fallback to docker, systemctl or a shell
...
// its value is real. `dagu_run_id` is carried as null until then.
// =====================================================
...
var VERSION = '1.0.0';
var HELPER = '/usr/local/sbin/mythos-hostops';
...
// opts (tests only): {spawn, guardGate, allowlist_path, helper}
function invoke(payload, opts) {
  opts = opts || {};
  var startedAt = Date.now();

  if (!enabled()) return refusal('HOSTOPS_INPUT', 'hostops is disabled (MYTHOS_HOSTOPS=off)');
  if (!payload || typeof payload !== 'object' || Array.isArray(payload)) {
    return refusal('HOSTOPS_INPUT', 'request body must be a JSON object');
  }
  var unexpected = Object.keys(payload).filter(function (k) { return REQUEST_FIELDS.indexOf(k) === -1; });
  if (unexpected.length) return refusal('HOSTOPS_INPUT', 'unexpected field: ' + String(unexpected[0]).slice(0, 40));

  var operation = payload.operation;
  if (typeof operation !== 'string' || !/^[a-z][a-z.\-_]{2,64}$/.test(operation)) {
    return refusal('HOSTOPS_INPUT', 'operation must be an allowlist verb or host.* operation name');
  }
  var ids = {};
  ...
  if (ids.__bad) return refusal('HOSTOPS_INPUT', 'field ' + ids.__bad + ' fails identity validation');

  // ---- allowlist + governance as declared -----------------------------
  var al = loadAllowlist(opts);
  if (!al || !al.operations) return refusal('HOSTOPS_ALLOWLIST_UNAVAILABLE', 'hostops allowlist unavailable; failing closed');
  ...
  if (!op) return refusal('HOSTOPS_UNKNOWN_OPERATION', 'operation "' + operation.slice(0, 64) + '" is not in the hostops allowlist');
  if (op.class !== 'READ') {
    return refusal('HOSTOPS_NOT_READ', 'operation ' + op.__name + ' is class ' + op.class + '; the executor hostops path executes READ operations only (v1)', { operation: op.__name, class: op.class });
  }

  // ---- argument validation (defense in depth; the helper re-validates) -
  var args = payload.arguments || {};
  if (typeof args !== 'object' || Array.isArray(args)) return refusal('HOSTOPS_INPUT', 'arguments must be an object');
  ...
  if (bad) return refusal('HOSTOPS_ARG_INVALID', bad, { operation: op.__name });

  // ---- Resource Guard admission (before any process exists) -----------
  ...
  // ---- the boundary ----------------------------------------------------
  var argv = [ '-n', opts.helper || HELPER, verb ];
  Object.keys(args).forEach(function (k) { argv.push('--' + k, args[k]); });
  if (ids.task_id) argv.push('--task-id', ids.task_id);
  if (ids.othmode_task_id) argv.push('--othmode-task', ids.othmode_task_id);
  if (ids.github_task_id) argv.push('--github-task', ids.github_task_id);

  var spawn = opts.spawn || function (bin, a) {
    return cp.spawnSync(bin, a, { timeout: TIMEOUT_MS, maxBuffer: 1024 * 1024, encoding: 'utf8' });
  };
  var r = spawn(SUDO, argv);
  ...
  if (r.error && (r.error.code === 'ETIMEDOUT' || r.signal === 'SIGTERM')) {
    ...
    outcome = refusal('HOSTOPS_UNAVAILABLE', 'the hostops boundary could not be invoked: ' + String(r.error.code || r.error.message).slice(0, 80), { operation: op.__name });
  } else {
    var body = null;
    try { body = JSON.parse(String(r.stdout || '')); } catch (e) { /* malformed */ }
    if (!body || typeof body !== 'object') {
      // sudoers not installed / password prompt refused by -n lands here too
...
function describe() {
  ...
  return { version: VERSION, enabled: enabled(), helper: HELPER, read_operations: reads, dagu: 'not in the READ path by design (docs/MYTHOS_HOSTOPS_INTERFACE.md; HOSTOPS-1 decision)' };
}

...
function describe() {
  ...
  return { enabled: c.enabled, token_present: t.present, token_problem: t.present ? null : t.reason, max_body_bytes: c.maxBody, providers: providerRegistry.ids(), route: '/hooks/<provider>', capabilities: providerRegistry.all().reduce(function (a, p) { a[p.id] = p.capabilities(); return a; }, {}) };
}
...
function handle(req, res, deps) {
  ...
  if (!caps.signed_webhooks) {
    ...
    var pre = provider.verifyWebhook(req, { query: u.query, expectedToken: token.value });
    if (!pre.ok) { log({ level: 'warn', receiver: 'unauthorized', reason: pre.reason, provider: provider.id, request_id: deps.requestId }); return send(res, 401, { ok: false, error: 'unauthorized', reason: pre.reason }); }
  }
  return readBody(req, c.maxBody).then(function (raw) {
    if (caps.signed_webhooks) {
      // signed providers (Meta, future): verification needs the raw body; the provider owns the algorithm
      var v = provider.verifyWebhook(req, { query: u.query, rawBody: raw, expectedToken: token.present ? token.value : null, secret: deps.secretFor ? deps.secretFor(provider.id) : null });
      if (!v.ok) { log({ level: 'warn', receiver: 'unauthorized', reason: v.reason, provider: provider.id, request_id: deps.requestId }); return send(res, 401, { ok: false, error: 'unauthorized', reason: v.reason }); }
    }
    var body;
...
module.exports = { handle: handle, describe: describe, config: config, readToken: readToken, TOKEN_HEADER: TOKEN_HEADER, registry: providerRegistry };

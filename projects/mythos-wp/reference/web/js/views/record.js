...
import { GENERIC } from './resource.js';

function projectQs(ctx, r) { const needs = r.scope === 'catalog' || (r.scope === 'wp' && !r.global); return ctx.api.qs({ project: needs ? ctx.projectId() : undefined }); }
function lookup(ctx) { return (resource, q, display, by) => ctx.api.get('/api/r/' + resource + '/lookup' + ctx.api.qs({ project: ctx.projectId(), q, display, by })); }
function title(r, row) { return r.titleField && row[r.titleField] ? String(row[r.titleField]) : (r.singular || 'Record') + ' #' + row[r.idColumn]; }

function res(ctx, key) { return GENERIC.includes(key) ? ctx.resources()[key] : null; }
export async function render(main, params, query, ctx) {
  const r = res(ctx, params.resource);
...
export async function renderNew(main, params, query, ctx) {
  ...
  const form = recordForm({ resource: r, row: prefill, mode: 'create', lookup: lookup(ctx),
    onSubmit: async (payload) => { const o = await ctx.api.post('/api/r/' + r.key + projectQs(ctx, r), payload); toast((r.singular || 'Record') + ' created' + (o.audited ? '' : ' (audit write failed)'), o.audited ? 'ok' : 'warn'); dirtyGuard.dirty = false; navigate('#/r/' + r.key + '/' + encodeURIComponent(o.row[r.idColumn])); },
...
export async function renderEdit(main, params, query, ctx) {
  ...
  clear(box);
  const row = d.row;
  const back = '#/r/' + r.key + '/' + encodeURIComponent(row[r.idColumn]);
  ...
  const form = recordForm({ resource: r, row, mode: 'update', lookup: lookup(ctx),
    onSubmit: async (payload) => { const o = await ctx.api.patch('/api/r/' + r.key + '/' + encodeURIComponent(row[r.idColumn]) + projectQs(ctx, r), payload); toast('Saved: ' + (o.changed || []).join(', '), o.audited ? 'ok' : 'warn'); dirtyGuard.dirty = false; navigate(back); },

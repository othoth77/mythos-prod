...
// projects/idauto/ops/media-ops.js
//
...
// docs/IDAUTO_STORAGE_RUNBOOK.md.
//
...
var TOOL = 'projects/idauto/ops/media-ops.js';
...
function getStorageRoot() {
  ...
  if (!root) fail('IDAUTO_MEDIA_STORAGE_PATH is not set. See docs/IDAUTO_STORAGE_RUNBOOK.md.');
...
async function cmdBackup() {
  var root = getStorageRoot();
  var destBase = arg('dest') || '/home/deploy/backups';
...
var FORBIDDEN_DESTS = ['/', '/home', '/home/deploy', '/etc', '/var', '/usr', '/bin', '/boot', '/root', '/opt', '/srv'];

function assertSafeDest(dest) {
  var d = path.resolve(dest);
  if (FORBIDDEN_DESTS.indexOf(d) !== -1) {
...
function usage() {
  process.stdout.write([
    ...
    'Commands:',
    '  audit                                          read-only DB <-> filesystem integrity report',
    '  backup [--dest <dir>]                          create a verified backup (default /home/deploy/backups)',
    ...
    'restores over the live media store. See docs/IDAUTO_STORAGE_RUNBOOK.md.'
  ].join('\n') + '\n');
}

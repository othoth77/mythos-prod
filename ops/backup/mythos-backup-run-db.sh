#!/usr/bin/env bash
# ops/backup/mythos-backup-run-db.sh — scheduled entry point for a
# DATABASE-ONLY off-host backup pipeline (e.g. mythos_erp), mirroring
# ops/backup/mythos-backup-run.sh exactly except for media handling, which
# this pipeline has none of.
#
# WHY A SEPARATE WRAPPER, NOT A FLAG ON THE EXISTING ONE
# --------------------------------------------------------
# mythos-backup-run.sh hardcodes `--media "$MYTHOS_BACKUP_MEDIA_DIR"` on its
# `stage` call and requires MYTHOS_BACKUP_MEDIA_DIR to be set at all — both
# load-bearing for idauto, which does have a media store to reconcile. A
# database-only service has none (see
# ops/backup/mythos-backup-capture-db.sh's header for the full rationale),
# so this is a parallel file, not a mode flag: the existing wrapper's
# behavior for idauto is completely unmodified by this file's existence.
#
# This is a THIN SCHEDULED WRAPPER around the same pre-existing, audited
# tooling at projects/infrastructure/ops/offhost-backup.js that
# mythos-backup-run.sh wraps. It implements no backup logic of its own.
# Exit codes follow the tool: 0 clean, 1 usage/env, 2 anomaly, 3 refused.
#
# Modes:
#   backup        stage (no --media) -> manifest -> verify-local -> push -> verify-remote
...
# atomically to $MYTHOS_BACKUP_HEALTH_FILE, in its own file distinct from the
# idauto pipeline's, so the two never overwrite each other's status. Secrets
# are never read by this script; the adapter reads its own 0600 config.
# Nothing here prints secret values, and deletion is structurally refused by
# the underlying tool.
set -u -o pipefail
...
CONFIG_FILE="${MYTHOS_BACKUP_DB_CONFIG:-$HOME/.config/mythos/backup-schedule-db.env}"
...
write_health() {
  ...
  local code="$1" err="${2:-}"
  local health="${MYTHOS_BACKUP_HEALTH_FILE:-$HOME/mythos-backups/health/backup-health-db.json}"
  ...
  local tmp="$health.tmp.$$"
  {
    ...
    printf '  "source": "ops/backup/mythos-backup-run-db.sh",\n'
    printf '  "mode": "%s",\n' "${MODE:-unknown}"
    printf '  "status": "%s",\n' "$status"
    printf '  "exit_code": %s,\n' "$code"
    printf '  "started_at": "%s",\n' "$STARTED_AT"
...
run_step() {
  ...
  local label="$1"; shift
  local out st
  echo "[mythos-backup-db] step: $label"
  out="$(node "$TOOL" "$@" 2>&1)"
  st=$?
  if [ "$st" -ne 0 ]; then
    EXIT_CODE=$st
    LOG_TAIL="$label: $(printf '%s' "$out" | tail -n 3)"
    echo "[mythos-backup-db] FAILED at $label (exit $st)" >&2
    printf '%s\n' "$out" | tail -n 10 >&2
    return 1
  fi
  return 0
...
case "$MODE" in backup|verify|restore-test) ;; *)
  echo "Usage: mythos-backup-run-db.sh <backup|verify|restore-test>" >&2
  exit 1 ;;
esac
...
# Config declares WHAT to back up and WHERE staging lives — never
# credentials. Required: MYTHOS_BACKUP_DB_DIR, MYTHOS_BACKUP_STAGE_ROOT,
# MYTHOS_BACKUP_PREFIX — deliberately NOT MYTHOS_BACKUP_MEDIA_DIR: this
# pipeline has no media component, so there is nothing to require here.
# Optional: MYTHOS_BACKUP_HEALTH_FILE, MYTHOS_BACKUP_HOST
# shellcheck disable=SC1090
. "$CONFIG_FILE"
for v in MYTHOS_BACKUP_DB_DIR MYTHOS_BACKUP_STAGE_ROOT MYTHOS_BACKUP_PREFIX; do
...
if [ "$ok" -eq 1 ]; then
  write_health 0 ""
  echo "[mythos-backup-db] $MODE completed clean"
  exit 0
else

...
window.addEventListener('load', function() {
  ...
  // 1. La zone dash-taches-zone est maintenant dans le HTML (index.html)
  //    Pas besoin de l'injecter dynamiquement.

  // 2. Ajouter bouton "Mes Tâches" dans le header calendrier
  (function() {
    var calHeader = document.querySelector('#view-calendrier .page-header > div');
    if (calHeader) {
  ...
  // 3. Patcher renderCalendrier pour intégrer les tâches triées
  ...
  // 4. Patcher updateDashboardStats pour le widget
  if (typeof updateDashboardStats === 'function') {
  ...
  // 5. Patcher showView pour la page #tache
  ...
  // Si l'URL commence par #tache au chargement
  if (location.hash === '#tache') renderTachePage();
});

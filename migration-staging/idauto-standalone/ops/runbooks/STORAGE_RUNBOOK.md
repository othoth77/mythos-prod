...
| Off-host copy | **RESOLVED 2026-08-14 — this row was stale as migrated and is corrected here.** A verified off-host copy of the database exists in object storage: dump → SHA-256 (C1) → upload → fresh download → SHA-256 (C2) → C1==C2 → isolated restore, 24 tables / 2,551 rows source-identical. See [`OFF_HOST_BACKUP_GATE.md`](OFF_HOST_BACKUP_GATE.md) §6. **Still outstanding:** recurring scheduling, retention automation, and off-host coverage of the *media store* specifically — the verified batch covered the database. |

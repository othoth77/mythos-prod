...
async function run() {
  ...
  section('H. OTHMODE discovery — five states, nothing leaks');
  {
    ...
    ok(gh && gh.source === 'mcp-capabilities' && gh.capabilities.length === 3 && gh.enabled === false && gh.direction === 'outbound', 'the outbound github capability server is rendered with its 3 tools and enabled:false');

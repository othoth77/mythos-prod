...
var NEW_TABLES = ['wp_inboxes', 'wp_contacts', 'wp_conversations', 'wp_messages', 'wp_message_attachments', 'wp_conversation_events', 'wp_tags', 'wp_contact_tags', 'wp_conversation_tags', 'wp_ai_runs', 'wp_ai_suggestions', 'wp_inbound_events', 'wp_contact_identities', 'wp_inbox_routes', 'wp_routing_drops', 'wp_schema_migrations'];
...
function cleanup() {
  ...
  return migrate.status(pool).then(function (s) {
    ...
    s.applied.slice().reverse().forEach(function (v) { chain = chain.then(function () { return migrate.down(pool, v); }); });
    return chain;
...
cleanup()
  ...
  .then(function () { return migrate.up(pool); })
  .then(function (r) { ok(r.applied.indexOf('0001_comms_core') !== -1 && r.applied.indexOf('0002_inbound_events') !== -1 && r.applied.indexOf('0003_outbound') !== -1 && r.applied.indexOf('0005_identities_reconciliation') !== -1 && r.applied.indexOf('0006_shared_account_routing') !== -1, 're-apply after rollback (0001 … 0006)'); return migrate.up(pool); })

...
var apiPath = path.join(BASE, 'projects', 'idauto', 'reference', 'api.js');
...
if (STATIC_ONLY) {
  var dbPath = require.resolve(path.join(BASE, 'projects', 'idauto', 'reference', 'db.js'));
  require.cache[dbPath] = { id: dbPath, filename: dbPath, loaded: true, exports: {} };
}
var api = require(apiPath);
var storage = require(path.join(BASE, 'projects', 'idauto', 'reference', 'storage.js'));
var ingestion = require(path.join(BASE, 'projects', 'idauto', 'reference', 'ingestion.js'));
var pass = 0, fail = 0;
...
(async function main() {
  ...
  if (STATIC_ONLY) console.log('\nIDA3D_STATIC_ONLY=1: live database and HTTP cases were not run.');
    ...
    db = require(path.join(BASE, 'projects', 'idauto', 'reference', 'db.js'));
    await liveCases();
  }

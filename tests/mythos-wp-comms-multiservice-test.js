...
migrate.up(pool).then(wipe)
  ...
  .then(function (x) { ok(x.status === 201 && x.data.row.kind === 'automotive', 'V2: automotive project needs no database catalogue — it reads a Kitchen (' + x.status + ')'); return req('POST', '/api/r/projects', { id: 'dar-hijama', display_name: 'Dar Hijama', kind: 'service', status: 'active', currency: 'TND' }, 'own'); })
  .then(function (x) { ok(x.status === 201 && x.data.row.kind === 'service' && x.data.row.catalog_dsn_env === undefined, 'service project without catalogue created (legacy column hidden)'); return req('POST', '/api/r/projects', { id: 'mythos-prod', display_name: 'MYTHOS PROD', kind: 'internal', status: 'active', currency: 'TND' }, 'own'); })
  .then(function (x) { ok(x.status === 201 && x.data.row.kind === 'internal', 'internal project without catalogue created'); return q("INSERT INTO wp_projects (id, display_name, kind) VALUES ('bad-auto','x','automotive') RETURNING id").then(function (r) { ok(r.rows[0].id === 'bad-auto', 'db: V2 dropped the catalogue CHECK (Kitchen replaces it)'); return q("DELETE FROM wp_projects WHERE id = 'bad-auto'"); }); })
  .then(function () { store.invalidate(); return req('GET', '/api/meta', undefined, 'op'); })
  .then(function (x) { var kinds = {}; x.data.projects.forEach(function (p) { kinds[p.id] = p.kind; }); ok(kinds['auto-a'] === 'automotive' && kinds['dar-hijama'] === 'service' && kinds['mythos-prod'] === 'internal', 'meta carries project kind'); ok(x.data.projects.filter(function (p) { return p.id === 'dar-hijama'; })[0].kitchen === null, 'service project reports no kitchen'); })
  // ---- inboxes
  ...
  // the scope is not a read filter: a member may not WRITE into an inbox they do not belong to
  .then(function () { return req('POST', '/api/projects/auto-a/comms/conversations/' + ids.convA1 + '/read', {}, 'agent1'); })
  .then(function (x) { ok(x.status === 404, 'member cannot mark an A1 conversation read'); return req('PATCH', '/api/projects/auto-a/comms/conversations/' + ids.convA1, { status: 'resolved' }, 'agent1'); })
  .then(function (x) { ok(x.status === 404, 'member cannot change an A1 conversation'); return req('POST', '/api/projects/auto-a/comms/conversations/' + ids.convA1 + '/notes', { text: 'not mine' }, 'agent1'); })
  .then(function (x) { ok(x.status === 404, 'member cannot note on an A1 conversation'); return req('POST', '/api/projects/auto-a/comms/conversations/' + ids.convA1 + '/messages', { text: 'hello', client_ref: 'scope-test-1' }, 'agent1'); })
  .then(function (x) { ok(x.status === 404, 'member cannot reply in an A1 conversation'); return req('POST', '/api/projects/auto-a/comms/conversations/' + ids.convA2 + '/read', {}, 'agent1'); })
  .then(function (x) { ok(x.status === 200, 'member CAN work their own A2 conversation'); return q("SELECT count(*)::int AS n FROM wp_messages WHERE conversation_id = $1 AND client_ref = 'scope-test-1'", [ids.convA1]); })
  .then(function (r) { ok(r.rows[0].n === 0, 'no message row was written for the refused reply'); return q('SELECT handler, updated_at FROM wp_conversations WHERE id = $1', [ids.convA1]); })
  .then(function (r) { ids.a1Before = r.rows[0]; var dir = r.rows[0].handler === 'ai' ? 'ai_to_human' : 'human_to_ai'; return req('POST', '/api/projects/auto-a/comms/conversations/' + ids.convA1 + '/handoff', { direction: dir }, 'agent1'); })
  .then(function (x) { ok(x.status === 404, 'member cannot hand over an A1 conversation'); return req('GET', '/api/projects/auto-a/comms/conversations/' + ids.convA1 + '/handoffs', undefined, 'agent1'); })
  .then(function (x) { ok(x.status === 404, 'member cannot read A1 handoff history'); return q('SELECT handler, updated_at FROM wp_conversations WHERE id = $1', [ids.convA1]); })
  .then(function (r) { ok(r.rows[0].handler === ids.a1Before.handler && String(r.rows[0].updated_at) === String(ids.a1Before.updated_at), 'the refused handoff changed nothing'); return req('GET', '/api/search?q=Bonjour%20A1&project=all', undefined, 'op'); })
  .then(function (x) { var conv = (x.data && x.data.groups || []).filter(function (g) { return g.key === 'conversations'; })[0]; ok(x.status === 200 && conv && conv.items.some(function (i) { return String(i.id) === String(ids.convA1); }), 'unscoped search finds the A1 conversation (control)'); return req('GET', '/api/search?q=Bonjour%20A1&project=all', undefined, 'agent1'); })
  .then(function (x) { var conv = (x.data && x.data.groups || []).filter(function (g) { return g.key === 'conversations'; })[0]; ok(x.status === 200 && (!conv || conv.items.every(function (i) { return String(i.id) !== String(ids.convA1); })), 'member search never returns an A1 conversation'); })
  // ---- assistant on a service project (no catalogue) never fabricates
  .then(function () { return store.resolve('dar-hijama'); })
  .then(function (resolved) { ok(resolved && !resolved.catalogPool, 'service project resolves without a catalogue pool'); return assistant.suggest(pool, resolved, ids.convB1, 'op', {}); })

...
migrate.up(pool)
  ...
  .then(function (x) { ok(x.status === 200 && x.data.contact_wa_id === undefined && x.data.contact_masked === '***001' && x.data.unread_count === 2, 'detail carries the MASKED number only for a non-admin, plus unread'); return req('GET', '/api/projects/comms-test/comms/conversations/' + ids.conv + '/messages'); })
  ...
  .then(function (x) { ok(x.status === 200 && x.data.wa_id === undefined && x.data.wa_masked === '***001' && x.data.conversations.length === 1, 'contact record: masked number for a non-admin + conversations'); return req('PATCH', '/api/projects/comms-test/comms/contacts/' + ids.contact, { display_name: 'Ahmed B.', language: 'fr', notes: 'garage à Sfax' }); })

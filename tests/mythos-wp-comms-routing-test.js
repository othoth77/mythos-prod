...
migrate.up(pool).then(wipe)
  ...
  // ---------- privacy: a message the adapter REJECTS on a shared instance keeps no payload (redaction does not strip text/numbers)
  .then(function () { var bad = msg('bad id with spaces', '21655000123', 'REJECTED-PERSONAL-TEXT-1'); return hook(bad); })
  .then(function (x) { ok(x.status === 200 && x.body.accepted === false && x.body.reason === 'MESSAGE_ID', 'privacy: malformed message on the shared instance rejected (' + (x.body && x.body.reason) + ')'); var lidOnly = msg('R2', '21655000124', 'REJECTED-PERSONAL-TEXT-2', { lid: '777777777777777' }); delete lidOnly.data.key.senderPn; return hook(lidOnly); })
  .then(function (x) { ok(x.body.accepted === false && x.body.reason === 'SENDER_UNRESOLVED', 'privacy: LID-only sender on the shared instance rejected (' + (x.body && x.body.reason) + ')'); return q("SELECT count(*)::int AS n, count(payload)::int AS p FROM wp_inbound_events WHERE instance = $1 AND status = 'rejected' AND reason IN ('MESSAGE_ID', 'SENDER_UNRESOLVED')", [INST]); })
  .then(function (r) { ok(r.rows[0].n === 2 && r.rows[0].p === 0, 'privacy: rejected shared-instance events are recorded hash-only (' + r.rows[0].n + ' rows, ' + r.rows[0].p + ' payloads)'); return leak('REJECTED-PERSONAL-TEXT'); })
  .then(function (n) { ok(n === 0, 'privacy: rejected message content nowhere in the database'); return leak('21655000124'); })
  .then(function (n) { ok(n === 0, 'privacy: rejected sender number nowhere in the database'); var bad = msg('bad id on dedicated', '21655000125', 'dedicated rejected'); bad.instance = 'ded-inst'; return hook(bad); })
  .then(function () { return q("SELECT count(payload)::int AS p FROM wp_inbound_events WHERE instance = 'ded-inst' AND status = 'rejected' AND reason = 'MESSAGE_ID'"); })
  .then(function (r) { ok(r.rows[0].p === 1, 'compat: a dedicated instance keeps the redacted dead-letter payload (COMMS-2 semantics)'); })
